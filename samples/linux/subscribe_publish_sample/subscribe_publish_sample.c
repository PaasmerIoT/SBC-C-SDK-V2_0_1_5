...
extern void WriteBlue(int number, char *state);
extern int ReadBlue(int number);
extern void WifiWrite(char* feedname, int feedpin, char* state);
extern int WifiRead(char* feedname,int feedpin);
	...
	while(ptrs)
	{
		if(!strncmp(params->payload,ptrs->feedname,strlen(ptrs->feedname)))
		{
			...
			if(strstr(params->payload,"on"))
			{
				...
				else if(!strcmp(ptrs->ConnectionType,"BLE"))
					WriteBlue(ptrs->feedpin,"on");
				else if(!strcmp(ptrs->ConnectionType,"Wifi")) {
                                        WifiWrite(ptrs->feedname,ptrs->feedpin,"ON");
                                }
			...
			else if(strstr(params->payload,"off"))
			{
				if(!strcmp(ptrs->ConnectionType,"GPIO"))
					gpioWrite(ptrs->feedpin,0);
                                        ...
                                        WriteBlue(ptrs->feedpin,"off");
				else if(!strcmp(ptrs->ConnectionType,"Wifi")) {
                                        WifiWrite(ptrs->feedname,ptrs->feedpin,"OFF");
                                }
...
if(wifiDiscover == 1 || bluetoothDiscover == 1){
	pthread_t my_thread;
	int discover_size=1024;
	pthread_create(&my_thread,NULL,Discover,&discover_size);
}

int errorLoop = 0;
IoT_Error_t rc = FAILURE;
for(errorLoop = 0;errorLoop <= 10000;errorLoop++){
errorStatus = false;
while(!errorStatus){
...
if(((timedelay++)%timePeriod)==0){
	...
	char feed2Details[256],feed3Details[256],Credentials[325];
				...
				while(ptr){
					...
					else if(!strcmp(ptr->ConnectionType,"BLE"))
						feed1Value = ReadBlue(ptr->feedpin);
					else if(!strcmp(ptr->ConnectionType,"Wifi"))
                                                feed1Value=WifiRead(ptr->feedname,ptr->feedpin);
					else
					...
					if(ptr)
					{
						...
						if(!strcmp(ptr->ConnectionType,"GPIO"))
							feed2Value = gpioRead(ptr->feedpin);
                                                	...
                                                	feed2Value = ReadBlue(ptr->feedpin);
						else if(!strcmp(ptr->ConnectionType,"Wifi"))
                                                feed2Value=WifiRead(ptr->feedname,ptr->feedpin);
						else
					...
					if(ptr)
					{
						if(!strcmp(ptr->ConnectionType,"GPIO"))
							feed3Value = gpioRead(ptr->feedpin);
                                                	...
                                                	feed3Value = ReadBlue(ptr->feedpin);
						else if(!strcmp(ptr->ConnectionType,"Wifi"))
                                                feed3Value=WifiRead(ptr->feedname,ptr->feedpin);
						else
