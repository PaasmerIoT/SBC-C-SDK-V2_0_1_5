...
if [ -f details.h ] ; then 
	username=$(sudo cat details.h | grep "UserName" | awk '{print $3}' | tr -d '"')
	echo $username
	device=$(sudo cat details.h | grep "DeviceName" | awk '{print $3}' | tr -d '"')
        echo $device
else
...
download()
{
	...
	if [ $status = true ]; then
		...
		if [ -f config.py ] || [ -f config.properties ] ; then
                        echo " You Have entered a Different Language SDK Device name, Please enter C SDK Device name"
                        if [ -f config.py ]; then
                                sudo rm config.py
		...
		if [ -f details.h ]; then
                       	sudo cp config.h samples/linux/subscribe_publish_sample/config.h
                       	sudo mv config.h samples/tirtos/subscribe_publish_sample/cc3200/
		 else
